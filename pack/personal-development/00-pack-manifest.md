...
| PD.CAT.001 | Memes Catalog | CAT | ПЕРЕНЕСЕНО в SA.CAT.001 (единый каталог мемов, PACK-systems-art) — секция B «Непродуктивные мемы». Номера M-NNN сохранены. | superseded |
...
| PD.CAT.004 | Productive Memes Catalog | CAT | ПЕРЕНЕСЕНО в SA.CAT.001 (единый каталог мемов, PACK-systems-art) — секция A «Продуктивные мемы». Номера P-NNN сохранены. | superseded |
...
| PD.CHR.010 | Volition | CHR | Волевость (наличие воли): способность агента выдерживать намерение и следовать осознанному направлению перед лицом более доступных и приятных альтернатив; формируется через ограничения и регулярный труд; в мире алгоритмической стимуляции — дефицитный ресурс | active |
| PD.CHR.011 | Thinking Quality | CHR | Качество мышления как измеримая характеристика созидателя: способность сформулировать задачу, декомпозировать её и удержать контекст. Рентген через работу с ИИ — модель честно отражает качество того, что в неё принесли | draft |
| PD.CHR.012 | Ai Delegation Boundary Awareness | CHR | Осознанность границ делегирования ИИ: способность созидателя удерживать чёткие обоснованные границы между тем, что усиливается через ИИ, и тем, что остаётся исключительно в зоне личной компетентности и авторской ответственности | draft |
...
| PD.FAIL.067 | Five Structural Nodes of Adult Learning Disruption | FAIL | 5 структурных узлов срыва обучения взрослых: (1) мировоззрение — блокирующие мемы, (2) направление — нет связи с делом, (3) методы — неэффективные инструменты, (4) состояние — истощение и компенсационный режим, (5) связь с проектами — знания не замыкаются на практику. Отличие от FAIL.064: FAIL.064 — где именно когнитивный разрыв в моменте обучения; FAIL.067 — в каком структурном домене находится первопричина срыва | active |
| PD.FAIL.068 | Compensation Syndrome | FAIL | Синдром компенсации: агент использует развитие (учёбу, саморазвитие, курсы) как инструмент снижения тревоги или заглушения неудовлетворённости — не из подлинного интереса к созиданию. Три режима получения удовольствия (кайф/компенсация/созидание): компенсационное развитие выглядит как активность, но не создаёт устойчивого роста | active |
| PD.FAIL.069 | Fragility Under Learning Difficulty | FAIL | Хрупкость обучения: агент интерпретирует трудности, уныние и сопротивление как сигналы остановки, а не как сигналы роста. Трудность = остановиться. Антихрупкость: трудность = ресурс, уныние = сигнал что система сопротивляется изменению — именно здесь и происходит развитие | active |
| PD.FAIL.070 | Conceptualization Without Grounding | FAIL | Концептуализация без заземления: освоение понятия останавливается на уровне цитирования и узнавания, без перехода к применению в материальной ситуации. Понятие используется как ярлык, а не как инструмент. Симптом: правильные слова при отсутствии соответствующих действий. | active |
| PD.FAIL.071 | Method Collecting Without Culture | FAIL | Человек регулярно осваивает новые методы (читает руководства, проходит курсы, делает заметки), но не меняет собственный стиль жизни. Внешне — активное развитие; реально — коллекционирование без интеграции. Корневая причина — психологический барьер: принять культуру = признать прежний образ жизни неполноценным (эффект Земмельвейса) | draft |
| PD.FAIL.072 | AI Thinking Atrophy | FAIL | Постепенный переход режима работы с ИИ: «ИИ помогает мне думать» → «ИИ думает вместо меня в очевидных случаях» → «ИИ принимает решения, потому что у него больше контекста» → «я исполняю предложенное». Конечная стадия — потеря самостоятельной способности к мышлению в делегированных областях | draft |
| PD.FAIL.073 | Object Myth — O-O as 'Natural' Perception | FAIL | Failure mode из BORO Methodology. | active |
| PD.FAIL.074 | Schema Reflects Reality Directly (False Reflection Assumption) | FAIL | Failure mode из BORO Methodology. | active |
...
| PD.FORM.089 | Learner Rcs | FORM | Role Characterisation Space (RCS) Ученика: болид-онтология (Пилот+Машина), 4 узла ЛР (Мыслительное/Саморазвитие/Управление IWE/Создание IWE), 7 bh-характеристик Аттестатора (bh.sys/inv/met/awr/agn/scl/stb) + 13 cp-срезов Диагноста (cp.rhy/wld/skl/iwe/cre/knw/env/int/auto/exo/agt/cult/traj). Ступень = двойной gate: bh-сигнал (Аттестатор, авто) → cp-подтверждение (Диагност, по запросу). Две оси мастерства IWE — cp.iwe (Пользователь 1-3) и cp.cre (Разработчик 1-3) — рекомендательные, не блокируют ступень. Формализм: FPF A.19 U.CharacteristicSpace | draft |
...
| PD.FORM.092 | Lr As Mass Program | FORM | ЛР как массовая программа: триада методология+платформа+адаптивная персонализация делает индивидуальный путь роста интеллекта масштабируемым — без наставника 1-к-1, с обещанием мировоззренческого сдвига по дуге PD.FORM.087. Содержит 8 открытых гипотез с revision criteria и 6 антипаттернов применения | draft |
| PD.FORM.095 | Artifact Role Accounting | FORM | Event Model для артефактов созидателя: 4 репозитория пользователя × роль исполнителя × срезы RCS. Классификатор тип-артефакта → роль → срез. Различение учебного артефакта (ДЗ, заметки) и созидательного (публикация, проект). Алгоритм автоклассификации через субагент + ручная верификация пограничных случаев. Связь с RCS (FORM.089): какой тип артефакта → какой срез M1/M2/M3/M4/W. | active |
| PD.FORM.096 | Three By Three Areas And Roles | FORM | Таблица 3×3: три области интересов (предметная область, метод работы, команда) × три деятельностные роли (предприниматель, инженер, менеджер) — инструмент самооценки и планирования развития для Систематического ученика и выше | stable |
| PD.FORM.097 | Worldview As Executable Code | FORM | Мировоззрение как операционная модель: 4 блока — (1) мировоззрение = исполняемый код (декларация vs исполнение); (2) ценности = инварианты, проверяемые конфликтом; (3) личность = 4-слойная вычислительная система (код/ОС/процессор/среда); (4) минимальный стандарт программы ЛР = 10ч+10$/нед | stable |
| PD.FORM.098 | Style As Lr Material | FORM | Стиль как единая характеристика созидателя — материал, на котором ставится ЛР. Развилка «внешнее vs внутреннее» (разгрести хаос / развить себя) — ложная оппозиция. Стиль един во всех сферах и ставится на любом доступном материале | active |
| PD.FORM.099 | Inner Markers Of Intellectual Development | FORM | Каталог 7 внутренних маркеров развития интеллекта (ясность, стрессоустойчивость cognitive, калиброванная уверенность, чувство нестыковки, лёгкость переключения уровней, чувство масштаба незнания, любопытство как фон). Третья оптика измерения в дополнение к auto-events и рабочим продуктам PD.FORM.093. Индикаторы subject-internal, не дублёры характеристик выпускника | draft |
| PD.FORM.100 | It Delivery Pipeline | FORM | Конвейер доставки ИТ-программы от продакт-менеджера до клиента: вход–действие–выход, роль Docker/PR/Kubernetes | active |
| PD.FORM.101 | Grounding | FORM | Заземление: навык и норма соединения абстрактного понятия с конкретной ситуацией из физического мира. Антипод режима «концептуализация без заземления» (PD.FAIL.070). Без заземления понятие остаётся словесным — невозможно проверить, освоено оно или заучено. | stable |
| PD.FORM.102 | Thinking As Modeling | FORM | Мышление в системном смысле — это построение моделей объекта, а не цитирование фактов. Модель строится из понятий и связей; набор приёмов моделирования и набор понятий определяют, какая именно модель получится. Один объект порождает разные модели у разных моделеров. | stable |
| PD.FORM.103 | Guide Development Pipeline | FORM | Конвейер разработки и изменения универсального руководства: 10 этапов вход-действие-выход от концепции до прода, аналог PD.FORM.100 | active |
| PD.FORM.104 | Iwe Multiplier | FORM | Мультипликатор IWE: отношение бюджетных часов закрытых РП к часам учётного времени за период. Меряет AI-leverage пилота. | active |
| PD.FORM.106 | Worker Stages | FORM | Четыре степени квалификации, по которым программа Рабочего развития (РР) проводит человека от Ученика (выход ЛР) к Мастеру: Работник → Стратег → Специалист → Практик с выходом на Мастера. 10 руководств R1-R10 — состав материала программы; жёсткой привязки руководство↔степень нет. Развивает роли Интеллектуала и Профессионала. Уточняет терминологию: ступени = у Ученика; степени квалификации — для остальных ролей | draft |
| PD.FORM.107 | Worker Stage Matrix | FORM | Нормативная матрица для программы Рабочего развития: 4 степени квалификации в фокусе программы (Работник → Стратег → Специалист → Практик) + выходная степень Мастер × ключевые направления развития. Для каждой степени: текущее состояние человека на входе, целевое состояние на выходе, индикатор. Аналог PD.FORM.080 для программы РР. Используется Диагностом, Портным, Навигатором при ведении человека по программе РР. ВАЖНО: применяется термин «степень квалификации», не «ступень РР» (см. D.164) | draft |
| PD.FORM.109 | Professional Graduate | FORM | Каталог характеристик выпускника программы РР на степени квалификации Мастер (ШСМ 6) — наблюдаемые свойства Профессионала на выходе. 8 характеристик с целевыми baseline-индексами, источниками данных, связью с шедевром (MIM.WP.002) и резидентурой (MIM.FMT.003). Двойной gate выпуска: метрический + предъявленный шедевр + успешная защита. Аналог PD.FORM.093 для роли Профессионала | draft |
| PD.FORM.110 | Four Ai Delegation Boundaries | FORM | 4 класса границ делегирования ИИ: финансы/медицина (компетентность), решения с последствиями (авторство), публичные публикации (голос), личные отношения (близость). Принцип: граница проводится не из страха, а из намерения сохранить собственную компетентность или авторскую ответственность | draft |
| PD.FORM.111 | Five Cultural Shifts Ai Era | FORM | 5 культурных сдвигов ИИ-эпохи: от написания кода → к формализации намерения; от индивидуального мастерства → к оркестровке агентов; от хранения знаний в голове → к формализации опыта; от последовательной работы → к параллельной; от «я решаю» → к «я проектирую среду». Сдвиги культурные, не технологические: не знание трансформеров, а навыки формализации, менеджмента, системного мышления | draft |
| PD.FORM.112 | Elite | FORM | Сообщество личностей с высокой агентностью, мастерством и ресурсами, из которого формируются команды создания систем на уровне общества или человечества. ≠ богатство, ≠ власть, ≠ номенклатура | draft |
| PD.FORM.113 | Agency | FORM | Способность и готовность личности инициировать изменения, действовать на масштабе системы и нести ответственность за результаты. ≠ амбиции, ≠ активность, ≠ харизма | draft |
| PD.FORM.114 | Life Mastery | FORM | Совокупность методов и практик, обеспечивающих эффективное управление собственной жизнью как системой: здоровье, внимание, время, отношения, состояние. ≠ профессиональное мастерство, ≠ soft skills | draft |
| PD.FORM.115 | Happiness | FORM | Рукотворное состояние личности, при котором устранены неудовлетворённости или есть уверенность в их скором устранении. ≠ удача, ≠ комфорт, ≠ мгновенное удовольствие | draft |
| PD.FORM.116 | Great Idea | FORM | Масштабная идея, определяющая направление развития личности и деятельности на длительный горизонт. ≠ цель, ≠ план, ≠ мечта | draft |
| PD.FORM.117 | Resources | FORM | Совокупность средств, доступных агенту для достижения целей: время, капитал, связи, знания, технологии. ≠ богатство, ≠ власть, ≠ потребление | draft |
| PD.FORM.118 | Personality | FORM | Системная модель человека как агента: совокупность желаний, способностей и ресурсов, формирующих уникальную картину мира и поведение. ≠ человек (биологический), ≠ роль (функциональная), ≠ личный бренд | draft |
| PD.FORM.119 | Society | FORM | Системный уровень организации людей: совокупность сообществ, институтов и систем, объединённых территорией, культурой и governance. ≠ страна (политическая граница), ≠ государство (аппарат власти), ≠ народ (этнос) | draft |
| PD.FORM.120 | Humanity | FORM | Высший системный уровень: совокупность всех людей как вида, объединённых общей эволюционной судьбой, культурой и технологиями. ≠ человечество как сумма индивидов, ≠ цивилизация (культурный слой), ≠ планета Земля | draft |
| PD.FORM.121 | Meaning | FORM | Смысл жизни — осознанно созданный агентом ориентир, придающий направление действиям и связывающий личные впечатления с созданием успешных систем. ≠ предназначение (данное свыше), ≠ цель (конкретный результат), ≠ миссия (социальная роль) | draft |
| PD.FORM.122 | Freedom | FORM | Свобода воли — способность агента осознанно корректировать выбор через стоп-момент, увеличивая объём учитываемой информации. ≠ произвол, ≠ отсутствие ограничений, ≠ абсолютная автономия | draft |
| PD.FORM.123 | Practice | FORM | Практика — применение метода в реальной деятельности с получением обратной связи и формированием навыка. ≠ теория, ≠ метод (как описание), ≠ однократное действие | draft |
| PD.FORM.124 | Upbringing | FORM | Воспитание — осознанная передача мировоззрения (мета-моделей мышления, принципов, ценностей) от одного поколения к другому. ≠ обучение (знания), ≠ мораль (нормы поведения), ≠ генетическое наследство | draft |
| PD.FORM.125 | Family | FORM | Семья — система передачи: генетическое наследие + материальный достаток + мемы (мировоззрение). ≠ романтические отношения, ≠ соседство по прописке, ≠ финансовая единица | draft |
| PD.FORM.126 | Community | FORM | Сообщество — группа единомышленников, объединённых общими интересами, целями или деятельностью, служащая средой для обмена знаниями, поддержки и совместного создания систем. ≠ толпа, ≠ аудитория, ≠ социальная сеть | draft |
| PD.FORM.127 | Systemic Self Development | FORM | Проект-метод выстраивания себя как системы через итерации и инкременты | draft |
| PD.FORM.128 | Negative Utility Of Labor | FORM | Экономическая теория поведения: объяснение прокрастинации через уравнение «выигрыш vs затраты» | draft |
| PD.FORM.129 | Meme | FORM | Единица культурной передачи: идея, поведение, стиль | draft |
| PD.FORM.130 | Memetics | FORM | Дисциплина культурной передачи через единицы культуры | draft |
| PD.FORM.131 | Cyberhuman | FORM | Человек + экзокортекс + ИИ как единая система | draft |
| PD.FORM.132 | Knowledge Compilation | FORM | Перевод знаний в мировоззрение через цикл «прочитал → применил → отразил → повторил» | draft |
| PD.FORM.133 | Bolid Audit | FORM | Диагностика компонентов Болида: Пилот + Машина (и связи между ними) | draft |
| PD.FORM.134 | Formality Spectrum | FORM | Диапазон от быстрого (S1) к медленному (S2) мышлению | draft |
...
| PD.METHOD.019 | Creative Pipeline | METHOD | Творческий конвейер: маршрут от сырого сигнала через разбор к завершённому артефакту — три культуры конвейера, цикл capture→draft→publish | active |
| PD.METHOD.020 | Hygienic Minimum | METHOD | Гигиенический минимум развития: шесть столпов (ритм, смысл, баланс, устойчивость, организованность, экзокортекс) — пороговое условие, ниже которого развитие невозможно | active |
| PD.METHOD.021 | Learner Culture | METHOD | Культура ученика как допуск к SoTA: пять установок (любопытство, незнание, черновик, ритм, передача), без которых новое знание не усваивается | active |
| PD.METHOD.022 | State Management | METHOD | Работа с состоянием: мониторинг, защита и развитие продуктивного потенциала — дельта к PD.FORM.038 §4.1; три слоя работы с состоянием как настраиваемым режимом | active |
| PD.METHOD.023 | Flow Management | METHOD | Управление тремя потоками созидателя (рабочие продукты / информация / энергия): теория ограничений применительно к личной системе — выявить узкое место, расширить, не оптимизировать остальные | active |
| PD.METHOD.024 | Ai Interaction Mastery | METHOD | Системный метод выстраивания продуктивного взаимодействия с ИИ как усилителем мышления: триада Контекст+Промпт+Запрос, модель как зеркало мировоззрения, три режима работы (ученик/профессор/команда), границы экзоскелета | active |
| PD.METHOD.025 | Planning Cascade | METHOD | Пятиуровневый метод планирования для Ученика (программа → квартал → месяц → неделя → день). Variant B: неделя = гипотеза роста, день = тактика под гипотезу. Обеспечивает каскадное уточнение без ежедневной регенерации ДЗ. | active |
| PD.METHOD.026 | Physical Anchors for Role Switching | METHOD | Использование физических объектов среды (стикер, доска, карточка) как якорей для переключения между ролями и активации нужного ритуала | active |
...
| PD.METHOD.034 | Grounding | METHOD | Практика связывания абстрактного понятия с конкретной ситуацией из своей работы. Применяется сразу после введения нового понятия, до перехода к следующему. Тест: можно ли найти 1 пример за неделю; уровни заземления 0-4 (PD.FORM.101 §3). | active |
| PD.METHOD.035 | Description Of What | METHOD | Дисциплинирующий приём формального мышления: для любого утверждения, понятия или модели — задать вопрос «описание чего это?». Возвращает мышление от уровня описания к уровню объекта. Применяется при чтении, обсуждении, проектировании моделей. | active |
| PD.METHOD.036 | Session Time Accounting | METHOD | Метод фиксации факта работы по сессии: оценка бюджета до начала + регистрация факта по итогам, привязка к РП. Специализация PD.METHOD.001 для дневной гранулярности. | active |
| PD.METHOD.037 | Conceptual Work | METHOD | Перевод термина (слова с определением) в понятие (узел в концепт-графе со связями). Применяется при освоении нового материала, при чтении, при создании Pack. Тест: ≥3 связи с другими понятиями + применимость в новой ситуации | draft |
| PD.METHOD.038 | Modelling | METHOD | Построение модели ситуации/системы вместо запоминания формулировки. Модель содержит объект, его части и связи, способна перестраиваться при изменении ситуации. Применяется в системном разборе, при работе с противоречиями, при стратегировании. Тест: модель перестраивается, цитата — нет | draft |
| PD.METHOD.039 | Epistemic Rhythm | METHOD | Регулярная работа Интеллектуала с собственным знанием: проверка допущений, фиксация нестыковок, формулирование открытых вопросов, ревизия мировоззрения. Аналог тайм-учёта (METHOD.001), но для эпистемического слоя. Применяется при чтении, рефлексии, разборе с наставником | draft |
| PD.METHOD.040 | Transfer Of Learning | METHOD | Применение принципа, освоенного в одном домене, к задаче в новом домене. Тест зрелости Интеллектуала на ст. Специалист ШСМ. Не путать с transfer learning (ML) — это педагогический перенос. Тест: ≥2 задокументированных случая переноса между принципиально разными доменами | draft |
| PD.METHOD.041 | Mentor Partnership | METHOD | Регулярная работа Интеллектуала с наставником через разборы (MIM.FMT.004). Не учитель-ученик, а партнёрство для калибровки мыслительной работы. Применяется на всех ступенях программы РР; центральная практика для прохождения защит квалификаций ШСМ | draft |
| PD.METHOD.042 | Stop Cycle | METHOD | Протокол прерывания тревожных пережёвываний мыслей: короткое дыхание → запись одного следующего шага → отложенное окно для переживаний. Предотвращает бесполезную растрату энергии | active |
...
| PD.PRINC.046 | Mental Model As Constraint | PRINC | Mental model как ultimate constraint: в знаниевой работе ментальная модель — труднейший вид ограничения | active |
...
| PD.ROLE.002 | Intellectual | ROLE | Вторая роль в траектории созидателя — носитель мыслительного мастерства. Объект внимания — мышление. Применяет понятия и методы в проектах, переносит теорию в практику, работает с моделями. Соответствует степеням квалификации ШСМ Работник→Стратег→Специалист. Развивается в программе Рабочего развития параллельно с Профессионалом | active |
...
| PD.WP.019.stage-1 | Stage 1 Random | WP | Заготовка руководства для ступени Случайного: первый шаг, одно действие, минимальный ритм | draft |
...
> *Auto-generated by `generate-map.py` on 2026-06-20*
